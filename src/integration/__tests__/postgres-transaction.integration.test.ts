...
live("real administrative reconciliation service rolls back failures and serializes concurrent requests", async () => {
  ...
  if (!/test/i.test(databaseName) || /(?:prod(?:uction)?|main)/i.test(databaseName)) {
    throw new Error("TEST_DATABASE_URL database name must identify an isolated test database");
  }
  const db = new PrismaClient({ datasourceUrl: url });
