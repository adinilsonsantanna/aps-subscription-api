...
export class NotificationSettingsService {
  constructor(private db: any = new PrismaClient()) {}
  ...
  async save(shopDomain: string, input: any) {
    ...
    const teamFrequency = String(input.teamFrequency); if (!FREQUENCIES.has(teamFrequency)) throw new Error("invalid_team_frequency");
    const current = await this.db.notificationSettings.findUnique({ where: { shopId: shop.id }, include: { activeSendingDomain: true } });
    const requestedDomain = domainFromEmail(fromEmail), keepActive = current?.activeSendingDomain?.sendingVerified && current.activeSendingDomain.domain !== requestedDomain;
    const toggles = Object.fromEntries(TOGGLES.map(name => [name, input[name] === true]));
    const saved = await this.db.notificationSettings.upsert({ where: { shopId: shop.id }, create: { shopId: shop.id, fromName, fromEmail, replyTo, teamEmails, teamFrequency, ...toggles }, update: { fromName, fromEmail, replyTo, teamEmails, teamFrequency, ...toggles, ...(keepActive ? {} : { activeSendingDomainId: current?.activeSendingDomain?.domain === requestedDomain ? current.activeSendingDomain.id : null }) }, include: { activeSendingDomain: true } });
