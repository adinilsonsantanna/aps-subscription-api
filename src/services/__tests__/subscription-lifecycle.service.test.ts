...
function setup(overrides: Record<string, unknown> = {}) {
  const subscription = { id: "aps-1", gateway: "stripe", externalId: "sub_123", shopifyContractId: null, status: "active", lastPaymentStatus: "succeeded", shop: { domain: "known.myshopify.com" }, ...overrides };
  const actions: any[] = [];
  const history: any[] = [];
  const prisma: any = {
    ...
    subscriptionLifecycleAction: {
      ...
      create: async ({ data }: any) => { const value = { id: `action-${actions.length + 1}`, externalStatus: null, ...data }; actions.push(value); return value; },
