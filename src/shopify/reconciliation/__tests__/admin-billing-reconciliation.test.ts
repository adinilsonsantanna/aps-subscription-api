...
test("dry-run keeps divergent billing and order timestamps with zero writes", async () => { const f = fixture(); const result = await f.service.execute({ ...input, dryRun: true }); assert.equal(result.status, "dry_run"); assert.equal(result.after.attempt.completedAt, "2026-08-27T17:28:45Z"); assert.equal(result.after.order.processedAt, "2026-08-27T17:28:51.161Z"); assert.equal(f.attempt.orderAmount, null); assert.equal(f.audits.length, 0); });
test("live rejects an attempt that is not pending before mutations", async () => { const f = fixture(); f.attempt.reconciliationStatus = "complete"; await assert.rejects(f.service.execute(input), (error: any) => error instanceof AdministrativeReconciliationError && error.code === "billing_attempt_not_pending"); assert.equal(f.attempt.orderAmount, null); assert.equal(f.audits.length, 0); });
test("dry-run may inspect an attempt that is not pending without mutations", async () => { const f = fixture(); f.attempt.reconciliationStatus = "complete"; const result = await f.service.execute({ ...input, dryRun: true }); assert.equal(result.status, "dry_run"); assert.equal(f.attempt.orderAmount, null); assert.equal(f.audits.length, 0); });
test("first execution preserves distinct billing and order timestamps and creates one audit", async () => { const f = fixture(); const result = await f.service.execute(input); assert.equal(result.status, "reconciled"); assert.equal(f.attempt.orderAmount, "50.19"); assert.equal(f.attempt.orderCurrencyCode, "BRL"); assert.equal(f.attempt.completedAt.toISOString(), "2026-08-27T17:28:45.000Z"); assert.equal(f.attempt.reconciliationStatus, "complete"); assert.equal(f.order.amount, "50.19"); assert.equal(f.order.processedAt.toISOString(), "2026-08-27T17:28:51.161Z"); assert.equal(f.cycle.status, "succeeded"); assert.equal(f.audits.length, 1); });
...
test("different orderProcessedAt participates in payload hash and conflicts", async () => { const f = fixture(); await f.service.execute(input); await assert.rejects(f.service.execute({ ...input, orderProcessedAt: "2026-08-27T17:28:52Z" }), (error: any) => error instanceof AdministrativeReconciliationError && error.code === "reconciliation_payload_conflict"); assert.equal(f.order.processedAt.toISOString(), "2026-08-27T17:28:51.161Z"); assert.equal(f.audits.length, 1); });
...
test("orderProcessedAt is required and never falls back to completedAt", async () => { const f = fixture(); const { orderProcessedAt: _missing, ...withoutOrderProcessedAt } = input; await assert.rejects(f.service.execute(withoutOrderProcessedAt), (error: any) => error instanceof AdministrativeReconciliationError && error.code === "invalid_shopify_state"); assert.equal(f.order.processedAt, null); assert.equal(f.audits.length, 0); });
