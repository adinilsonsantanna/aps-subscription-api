...
export class RetryEngineService {
  constructor(private prisma: PrismaClient | any = new PrismaClient(), private fetchFn: Fetch = fetch, private lifecycle: SubscriptionLifecycleService | any = new SubscriptionLifecycleService(prisma)) {}
  ...
  async run(batchSize = 10, now = new Date()): Promise<RetryMetrics> {
    ...
    for (const candidate of candidates) {
      ...
      catch (error) { metrics.failed++; await this.markUncertain(candidate.id, now, { errorCode: "worker_error", errorMessage: this.safeError(error) }); }
    }
    await this.recoverFinalActions(now, batchSize);
    return metrics;
  }
  ...
  private async handleUncertain(job: any, result: AppResult, now: Date, metrics: RetryMetrics) {
    ...
    if (count >= job.maxReconciliations || now >= deadline) { const settings = await this.settings(job.shopId); const action = settings.paymentFailureAction; await this.prisma.$transaction(async (tx: any) => { await tx.billingRetryJob.update({ where: { id: job.id }, data: { status: RetryJobStatus.EXHAUSTED, completedAt: now, leaseExpiresAt: null, reconciliationCount: count, lastReconciledAt: now, errorCode: "reconciliation_exhausted", errorMessage: result.errorMessage || "Shopify attempt could not be reconciled", finalAction: action } }); await tx.billingRetryCycle.updateMany({ where: { id: job.cycleId, finalActionStatus: "none" }, data: { finalAction: action, finalActionStatus: "pending", finalActionError: null } }); }); metrics.failed++; return; }
    await this.markUncertain(job.id, now, { externalAttemptId: result.billingAttemptId || job.externalAttemptId, reconciliationCount: count, firstUncertainAt: job.firstUncertainAt ?? now, lastReconciledAt: job.firstUncertainAt ? now : null, reconciliationDeadlineAt: deadline, errorCode: result.errorCode || "external_result_uncertain", errorMessage: result.errorMessage }); metrics.rescheduled++;
  }
  ...
  private async executeFinalAction(cycle: any, now: Date) {
    ...
    await this.prisma.$transaction(async (tx: any) => {
      ...
      if (settings.teamNotificationFrequency !== TeamNotificationFrequency.NEVER) await tx.notificationOutbox.upsert({ where: { idempotencyKey: `retry-notification:${cycle.id}:${action}` }, create: { shopId: cycle.shopId, cycleId: cycle.id, idempotencyKey: `retry-notification:${cycle.id}:${action}`, frequency: settings.teamNotificationFrequency, eventType: "retry_exhausted", payload: { subscriptionId: cycle.subscriptionId, action }, availableAt: this.notificationTime(settings.teamNotificationFrequency, now) }, update: {} });
